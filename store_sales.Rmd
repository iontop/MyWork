...
```{r}
df_train <- df_set[[90]] %>% filter(set == "train")
df_test <- df_set[[90]] %>% filter(set == "test")
...
set.seed(123)
df_folds <- bootstraps(df_analysis)
...
#sales를 기준으로 lag하도록 recipe에 줬는데 df_test에는 sales값을 찾아야 하는 것이라 에러 발생생
